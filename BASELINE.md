...
| incoming_command_timeout | **0.1 s** | Servo 层输入流超时——**VR 映射层发布频率必须 >10 Hz**（比 TARGET_POSE_TIMEOUT 严得多；2026-09-25 全参数审计确认的实际生效值） |
...
2. ✅ **映射层**（2026-09-25 完成）——ClutchPoseMapper 移植（17 项测试）+
   ...
   **Quest 3 首飞完成（2026-09-26/27）**：`quest_adapter_node.py`（relay ws →
   /quest/pose+joy 100Hz）+ `--input quest` 映射器 + 11 项验收语义已复用。
   已实测：Grip 接合→臂跟随→松开冻结。
   **Quest 浏览器推流条件（硬件级行为）**：仅"头显佩戴 + 浏览器前台"时推流；
   熄屏/摘下/切后台 = WebSocket 断、页面 JS 暂停——系统侧 stale 保护正确停发，
   表现为"放下头显 = 臂自动冻结"（安全特性）。操控时保持佩戴。
   **键位实锤（client.js L1466 原文）**：0=Trigger(模拟)、1=Grip(模拟)、2=摇杆按压、
   3=A/X、4=B/Y、5=thumbrest 触碰、6=menu。曾按指纹误定 Grip=7（实为 thumbrest
   触碰）→"不按离合也跟随"→臂被拖进自折叠陷阱（同早期实证机制），已改正。
   待调优：固定握姿偏置（替换逐接合轴对齐）、缩放降档 1:3、逐 tick 平滑。
   ⚠️ **操作铁律：键盘遥操作时绝不能让 MuJoCo viewer 窗口保持焦点**——
   viewer 里空格 = 暂停仿真：按住空格离合时焦点在 viewer 上会把 /clock 暂停，
   映射器/夹爪（use_sim_time）全部冻结，表现为"按了没反应"。看仿真用 RViz。
   （2026-09-25 用户实测踩坑：space+W 后"全部无反应 + /clock 停发"，
   另同场抓出键盘模式离合 bug：空格不在平移/旋转键集合里，`space in _held`
   永远为假——离合改用与脚本模式同一个持久电平。）
